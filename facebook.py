"""
    ...
    access token::
        ...
        >>> result = api["me/feed"].using("iuyfsUGyuas|332423423").get()

    Methods ``post()`` and ``delete()`` are also available on locations.

"""

# TODO: more exception differentiation
# TODO: OAuth API detalisation
...
def shell():
    ...
    p = optparse.OptionParser(usage="%s [OPTIONS] APP_ID APP_SECRET" % \
            sys.argv[0])
    p.add_option("-t", "--access-token", help="access token to use")
